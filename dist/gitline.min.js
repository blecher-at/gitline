...
var GitlineConfig = (function () {
    function GitlineConfig() {
        this.dotHeight = 6;
        this.dotWidth = 8;
        this.remoteOnly = false;
        this.avatars = [this.avatar_gravatar];
    }
    GitlineConfig.prototype.avatar_gravatar = function (email) {
        return "http://www.gravatar.com/avatar/" + CryptoJS.MD5(email.toLowerCase()) + "?s=16&d=404";
    };
    return GitlineConfig;
})();
/**
...
var Expandable = (function () {
    ...
    Expandable.extend = function (element) {
        ...
        extended.whenShort = function (innerHTML) {
            extended.innerHTML = innerHTML;
            extended.onmouseout = function (event) {
                extended.innerHTML = innerHTML + " ";
...
var GithubCommitProvider = (function (_super) {
    ...
    GithubCommitProvider.prototype.obtainAccessToken = function (callback) {
    ...
    GithubCommitProvider.prototype.loadForks = function (url) {
        var _this = this;
        jQuery.when(jQuery.getJSON(this.gitURL(url, "forks")), jQuery.getJSON(this.gitURL(url, "branches"))).then(function (forks, branches) {
            //this.baseCommits = commits[0].data;
            _this.processBranches(url, branches[0].data);
            _this.forks = forks[0].data;
            _this.loadBranches();
        });
    };
    ...
    GithubCommitProvider.prototype.processCommits = function (data) {
        ...
        data.map(function (data) {
            ...
            c.parenthashes = data.parents.map(function (x) { return x.sha; });
    ...
    GithubCommitProvider.prototype.assignHeads = function (commit) {
        var _this = this;
        commit.parents1 = commit.parenthashes.map(function (x) { return x; }); // copy array
...
var Gitline = (function () {
    function Gitline() {
        ...
        this.commits = {};
        this.headsMap = {};
        this.config = new GitlineConfig();
    }
    Gitline.prototype.addCommit = function (commit) {
        this.commits[commit.getFullSha()] = commit;
    };
    Gitline.prototype.addBranch = function (refname, commit, specifity) {
        var branch = new Branch(refname, commit, specifity);
        this.headsMap[refname] = branch;
    };
    Gitline.prototype.render = function () {
        var _this = this;
        ...
        this.al.thenSingle("Loading Data", function () {
        ...
        }).then("Loading Commits", function () { return Object.keys(_this.data); }, function (sha) {
            var commit = new Commit(_this, _this.data[sha]);
            _this.addCommit(commit);
        })
            .thenSingle("Building Graph", function () {
            _this.buildGraph();
        })
            .then("Drawing Labels", function () { return Object.keys(_this.commits); }, function (sha) {
            ...
            .then("Drawing Merges", function () { return Object.keys(_this.commits); }, function (sha) {
            var commit = _this.commits[sha];
            _this.drawReferences(commit);
        })
            .thenSingle("Resizing", function () {
            _this.graphicalPanel.style.width = indexToX(_this.maxX + 1) + "px";
            _this.graphicalPanel.style.height = _this.rootLabel.offsetTop + "px";
        }).start();
        window.onresize = function (event) {
            _this.al.then("Redrawing", function () { return Object.keys(_this.commits); }, function (sha) {
                var commit = _this.commits[sha];
                commit.view.redraw();
            }).thenSingle("Resizing", function () {
                _this.graphicalPanel.style.width = indexToX(_this.maxX + 1) + "px";
    ...
    Gitline.prototype.buildGraph = function () {
        var _this = this;
    ...
    Gitline.prototype.drawCommit = function (commit) {
        // Label
    ...
    Gitline.prototype.drawReferences = function (commit) {
        commit.view.addRelations();
        commit.view.redraw();
    };
    Gitline.prototype.drawLabel = function (commit) {
        var label = document.createElement('div');
        label.className = "commit-legend";
        // SHA Hash 
        var shortSha = commit.getShortSha();
        var fullSha = commit.getFullSha();
        var sha = Expandable.extend(document.createElement("span"));
    ...
    Gitline.prototype.drawIdentity = function (type, id) {
        var el = Expandable.extend(document.createElement("gitline-identity"));
        ...
        el.style.background = this.config.avatars.map(function (f) { return "url(" + f(id.email) + ") no-repeat"; }).join(", ");
    ...
    */
    Gitline.prototype.initBranches = function () {
    ...
    // Launching
    Gitline.prototype.fromJSON = function (jsonFile) {
        return this.fromProvider(new LocalGit2JsonProvider(jsonFile));
    };
    Gitline.prototype.fromProvider = function (commitProvider) {
        this.commitProvider = commitProvider;
        return this;
    };
    Gitline.prototype.renderTo = function (panel) {
    ...
    Gitline.prototype.withHeader = function (header) {
    ...
    Gitline.displayFatalError = function (message) {
        alert(message);
    };
    return Gitline;
})();
...
var CommitView = (function () {
    ...
    CommitView.prototype.addRelations = function () {
        ...
        allmerges.forEach(function (merge) {
            _this.lines.push(new Curve(_this.canvas)
...
var Commit = (function () {
    ...
    Commit.prototype.getIndex = function () {
